...
object PrinterCommands {
    ...
    private fun zpl(text: String, printer: PrinterConfig, copies: Int): ByteArray {
        ...
        val widthDots = mmToDots(printer.widthMm)
        val heightDots = labelHeightDots(printer, lines.size)
        return buildString {
            append("^XA\n^PW$widthDots\n^LL$heightDots\n^LH0,0\n")
            lines.forEachIndexed { index, line ->
                append("^FO16,${16 + index * 28}^A0N,24,24^FD${line.replace("^", " ").replace("~", " ")}^FS\n")
            }
    ...
    private fun cpcl(text: String, printer: PrinterConfig, copies: Int): ByteArray {
        ...
        return buildString {
            ...
            append("PW ${mmToDots(printer.widthMm)}\r\n")
            lines.forEachIndexed { index, line -> append("TEXT 0 2 16 ${16 + index * 28} $line\r\n") }
            append("FORM\r\nPRINT\r\n")
        }.toByteArray(Charsets.US_ASCII)
    }
    ...
    private fun epl(text: String, printer: PrinterConfig, copies: Int): ByteArray {
        ...
        val heightDots = labelHeightDots(printer, lines.size)
        return buildString {
            append("N\nq${mmToDots(printer.widthMm)}\nQ$heightDots,${mmToDots(printer.gapMm)}\n")
            lines.forEachIndexed { index, line ->
                append("A16,${16 + index * 28},0,3,1,1,N,\"${line.replace("\\", " ").replace("\"", "'")}\"\n")
            }
    ...
    private fun labelHeightDots(printer: PrinterConfig, lineCount: Int): Int =
        if (printer.tsplPaperMode.name == "CONTINUOUS") (32 + lineCount * 28).coerceAtLeast(80)
        else mmToDots(printer.labelHeightMm).coerceAtLeast(80)

    private fun mmToDots(mm: Int): Int = (mm.coerceIn(0, 320) * 8).coerceAtLeast(0)
